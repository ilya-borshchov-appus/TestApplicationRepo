...
//        }
    ...
    fileprivate func initLayout() {
        ...
        if (iPhone && !iPad){
            compatible = "Compatible witi iPhone and iPod touch."
        } else if (!iPhone && iPad){
            compatible = "Compatible with iPad."
        } else if(iPhone && iPad){
            compatible = "Compatible with iPhone, iPad and iPod touch."
        }
        
        self.compatibilityInfo.text = String(format: "Requires iOS %@ or later. %@", (self.selectedApp?.minVersion)!, compatible)
        self.ratingInfo.text = "Rated \((self.selectedApp?.contentAdvisoryRating)!)"
        
        
        guard let path = Bundle.main.path(forResource: "languages_isoa2", ofType: "csv") else {
            return
        }
